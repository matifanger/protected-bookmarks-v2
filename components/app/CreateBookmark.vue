<template>
    <div>
  <button @click="modalIsOpen = true" class="transition duration-100 block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center fixed right-0 bottom-0 mb-5 mr-5 z-45" type="button">
  Add bookmark
    </button>
  <!--Modal-->
...
<div v-if="modalIsOpen === true" class="fixed flex justify-center items-center md:h-full md:inset-0 animated fadeIn faster z-50 transition ease-in duration-700 animated fadeIn" style="transition: opacity 0.25s ease;" >
    <div @click="modalIsOpen = false" class="absolute w-full h-full bg-black opacity-50 z-40 cursor-default transition-none"></div>
    <div class="relative px-4 w-full max-w-2xl h-full md:h-auto z-50">
        <!-- Modal content -->
        <div @keyup.enter="createNewBookmark" @keydown.esc="modalIsOpen = false" tabindex="0" class="relative rounded-lg shadow" style="background-color: rgb(55 65 81/var(--tw-bg-opacity));">
